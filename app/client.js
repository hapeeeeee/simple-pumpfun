...
import { getOrCreateAssociatedTokenAccount, getAssociatedTokenAddressSync, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID } from "@solana/spl-token";
...
export async function main() {
    ...
    // --------------------------CreatePool Start -----------------------------
    {
        console.log("\n--------------------------CreatePool Start -----------------------------");
        // Token在链上的mint地址
        const [poolPda, poolBump] = PublicKey.findProgramAddressSync([Buffer.from("pool"), Buffer.from(metadata.id)], program.programId);
        const pool_token_account = await getOrCreateAssociatedTokenAccount(program.provider.connection, payerPair, // 创建token账户的付款者
    ...
    // --------------------------Mint Token To Pool Start -------------------------------
    {
        ...
        const [poolPda, poolBump] = PublicKey.findProgramAddressSync([Buffer.from("pool"), Buffer.from(metadata.id)], program.programId);
        const pool_token_account = await getAssociatedTokenAddressSync(metadatamint, // Token绑定的Mint地址
        poolPda // tokenAccount所有者的公钥
        );
        console.log("pool_token_account.address:", pool_token_account.toBase58());
        ...
        console.log(`  mint token to user https://explorer.solana.com/tx/${txHashMintToken}?cluster=devnet`);
    ...
    // --------------------------MintToken Start-------------------------------
    {
        ...
        // 目标钱包的token账户
        const to_token_account = await getOrCreateAssociatedTokenAccount(program.provider.connection, payerPair, // 创建token账户的付款者
        metadatamint, // Token绑定的Mint地址
    ...
    // --------------------------MintToken to User Start-------------------------------
    {
        ...
        // 目标钱包的token账户
        const to_token_account2 = await getOrCreateAssociatedTokenAccount(program.provider.connection, payerPair, // 创建账户的付款者
        metadatamint, // Token绑定的Mint地址
        UserPair.publicKey // 目标账户的公钥
        );
        console.log("UserPair.publickey:", UserPair.publicKey, "to_token_account2.address:", to_token_account2.address.toBase58());
        ...
        console.log(`  mint token to user https://explorer.solana.com/tx/${txHashMintToken2}?cluster=devnet`);
    }
    // --------------------------MintToken to User End-------------------------------
    // --------------------------BurnToken Start ----------------------------
    {
        const [mint, bump] = PublicKey.findProgramAddressSync([Buffer.from("mint"), Buffer.from(metadata.id)], program.programId);
        // 查找要burn的tokenaccount,如果不存在会报错
        const payer_token_account = await getAssociatedTokenAddressSync(metadatamint, // Token绑定的Mint地址
        payerPair.publicKey // tokenAccount所有者的公钥
        );
        let burnBalance;
        const balance = (await program.provider.connection.getTokenAccountBalance(payer_token_account)).value.uiAmount;
        ...
        console.log(`  https://explorer.solana.com/tx/${txHash}?cluster=devnet`);
        const postBalance = (await program.provider.connection.getTokenAccountBalance(payer_token_account)).value.uiAmount;
        console.assert(burnBalance == postBalance, "Compare balances, it must be equal");
    }
    // --------------------------BurnToken End ----------------------------
    // --------------------------User Burn Token Start ---------------------
    {
        const [mint, bump] = PublicKey.findProgramAddressSync([Buffer.from("mint"), Buffer.from(metadata.id)], program.programId);
        // 查找要burn的tokenaccount,如果不存在会报错
        const user_token_account = await getAssociatedTokenAddressSync(metadatamint, // Token绑定的Mint地址
        UserPair.publicKey // tokenAccount所有者的公钥
        );
        let burnBalance;
        const balance = (await program.provider.connection.getTokenAccountBalance(user_token_account)).value.uiAmount;
        burnBalance = balance / 2;
        const burn_tokens_params = {
            quantity: new BN(burnBalance * 10 ** metadata.decimals),
            id: DIFF_SEED,
        ...
        await program.provider.connection.confirmTransaction(txHash);
        console.log(`  https://explorer.solana.com/tx/${txHash}?cluster=devnet`);
        const postBalance = (await program.provider.connection.getTokenAccountBalance(user_token_account)).value.uiAmount;
        console.assert(burnBalance == postBalance, "Compare balances, it must be equal");
    }
    // --------------------------User Burn Token End ---------------------
