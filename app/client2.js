...
import { Connection, Keypair, Transaction, SystemProgram, sendAndConfirmTransaction, PublicKey, LAMPORTS_PER_SOL, SYSVAR_RENT_PUBKEY, } from "@solana/web3.js";
    ...
    // --------------------------MintTokenBatchWithCreateAta Start-------------------------------
    {
        ...
        for (let i = 0; i <= users_wallet_without_ata.length; i++) {
            if (users_wallet_without_ata[i] && users_token_account[i]) {
                remaining_accounts.push({ pubkey: users_wallet_without_ata[i], isWritable: true, isSigner: false });
                remaining_accounts.push({ pubkey: users_token_account[i], isWritable: true, isSigner: false });
            }
        }
        ...
        const tx = new Transaction()
            .add(ComputeBudgetProgram.setComputeUnitLimit({ units: 2000000 }), ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 1200000 }), await program.methods
